...
def main(argv):
    ...
    def process_input(X_input, X_input_):
        ...
        ATM_option = tf.slice(X_input, (0,3), (-1,1))
        ATM_delta = tf.slice(X_input, (0,4), (-1,1))
        ATM_gamma = tf.slice(X_input, (0,5), (-1,1))

        S_ = tf.slice(X_input_, (0,0), (-1,1))
        T_ = tf.slice(X_input_, (0,1), (-1,1))
        ATM_option_ = tf.slice(X_input_, (0,2), (-1,1))
        ATM_delta_ = tf.slice(X_input_, (0,3), (-1,1))
        ATM_gamma_ = tf.slice(X_input_, (0,4), (-1,1))
        return S, K, T, S_, T_, r, ATM_option, ATM_delta, ATM_gamma, ATM_option_, ATM_delta_, ATM_gamma_
    ...
    # FINN self-supervised learning delta-gamma-hedging policy
    @tf.function
    def loss(X_input, X_input_):
        S, K, T, S_, T_, r, ATM_option, ATM_delta, ATM_gamma, ATM_option_, ATM_delta_, ATM_gamma_ = process_input(X_input, X_input_)
        ...
        gamma = tape.gradient(delta, S)
        gamma = tf.maximum(gamma, 0) # call/put gamma bounds
        
        beta = gamma / ATM_gamma
        alpha = delta - beta * ATM_delta
        V = -out + alpha * S + beta * ATM_option
        V_ = -out_ + alpha * S_ + beta * ATM_option_
        return hedging_mse(V, V_)
    ...
    def ATM_bs(S, K, T, r, sigma):
        ...
        if FLAGS.option_type == 'call':
            return call, call_delta, call_gamma
        elif FLAGS.option_type == 'put':
            return put, put_delta, put_gamma
    ...
    def get_batch2(stock_path,n, moneyness_range = (.5,2)):
        ...
        S_underlying = stock_path[picks]
        S_underlying_ = stock_path[picks+1]
        K = np.random.uniform(*moneyness_range, (n,1))*S_underlying
        ATM_option, ATM_delta, ATM_gamma = ATM_bs(S = S_underlying, K = S_underlying, T = FLAGS.hedge_option_ttm/250, r = 0, sigma = FLAGS.volatility)
        ATM_option_, ATM_delta_, ATM_gamma_ = ATM_bs(S = S_underlying_, K = S_underlying, T = (FLAGS.hedge_option_ttm-1)/250, r = 0, sigma = FLAGS.volatility)
        X = np.hstack([S_underlying, K, T/250, ATM_option, ATM_delta, ATM_gamma])
        X_ = np.hstack([S_underlying_, (T-1)/250, ATM_option_, ATM_delta_, ATM_gamma_])
        return X, X_
    ...
    ann.save('gbm_european_{}_{}_{}_{}.h5'.format(FLAGS.option_type, FLAGS.volatility, FLAGS.set_seed, FLAGS.hedge_option_ttm))
    ...
    plt.xlim([0, count])
    plt.savefig('gbm_european_{}_{}_{}_{}_loss.png'.format(FLAGS.option_type, FLAGS.volatility, FLAGS.set_seed, FLAGS.hedge_option_ttm))
