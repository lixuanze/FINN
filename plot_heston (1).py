...
def main(argv):
    ...
    model = tf.keras.models.load_model('heston_european_{}_{}_{}.h5'.format(FLAGS.option_type, FLAGS.volvol, FLAGS.set_seed))
    heston_results = pd.read_csv('heston_results/heston_model_results_{}.csv'.format(FLAGS.volvol))
    ...
    for j in range(len(t)):
        ...
        if not os.path.isdir('heston_results/TTM: ' + str(T)):
            os.makedirs('heston_results/TTM: ' + str(T))
        ...
        ax.set_title('Estimated Call Option Price for Given S and K (Heston)', fontsize=32, fontweight='bold')
        ...
        ax.set_title('Estimated Hedge Ratio ($\Delta$) for Given S and K (Heston)', fontsize=32, fontweight='bold')
        ax.xaxis.pane.fill = False
        ax.yaxis.pane.fill = False
        ax.zaxis.pane.fill = False
        ax.xaxis.pane.set_edgecolor('w')
    ...
    filename = 'heston_results_{}_{}_{}_.csv'.format(FLAGS.option_type, FLAGS.volvol, FLAGS.set_seed)
